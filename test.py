...
parser.add_argument('--volume_path', type=str, default='../data/Synapse', help='root dir for validation volume data')
...
if args.dataset == "Synapse":
    if os.path.basename(args.volume_path) != "test_vol_h5":
...
def inference(args, model, test_save_path=None):
    ...
    for i_batch, sampled_batch in tqdm(enumerate(testloader)):
        ...
        mv = np.mean(metric_i, axis=0)
        logging.info('idx %d case %s mean_dice %f mean_hd95 %f mean_asd %f' % (i_batch, case_name, mv[0], mv[1], mv[2]))

    metric_list = metric_list / len(db_test)
    for i in range(1, args.num_classes):
        logging.info('Mean class %d mean_dice %f mean_hd95 %f mean_asd %f' % (i, metric_list[i-1][0], metric_list[i-1][1], metric_list[i-1][2]))
    
    performance = np.mean(metric_list, axis=0)[0]
    mean_hd95 = np.mean(metric_list, axis=0)[1]
    mean_asd = np.mean(metric_list, axis=0)[2] 
    logging.info('Testing performance: mean_dice : %f mean_hd95 : %f mean_asd : %f' % (performance, mean_hd95, mean_asd))
...
if __name__ == "__main__":
    ...
    args.z_spacing = dataset_config[args.dataset]['z_spacing']
...
# 2. 创建一个新的字典来存改名后的权重
    ...
    # 规则 2: 替换内部模块名 MSCA 变成 msdc
        new_key = new_key.replace('MSCA', 'msdc')
    
        new_state_dict[new_key] = v

# 3. 将改名后的新权重加载进网络
# 【关键！】加上 strict=False，这样即使有多余的 conv33conv33conv11 废弃参数，也不会报错卡死
    msg = net.load_state_dict(new_state_dict, strict=False)
    print("✅ 权重名字二次修正并加载成功！被原作者废弃的冗余层已自动忽略。")
    print("Loaded MMRSG-UNet weights:", msg)
    
    log_folder = os.path.join(args.output_dir, 'test_log')
    os.makedirs(log_folder, exist_ok=True)
    logging.basicConfig(filename=os.path.join(log_folder, "test_log.txt"), level=logging.INFO, format='[%(asctime)s.%(msecs)03d] %(message)s', datefmt='%H:%M:%S')
