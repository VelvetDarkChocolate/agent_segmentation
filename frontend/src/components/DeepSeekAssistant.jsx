  ...
  async function runSegmentation() {
    ...
    try {
      ...
      setMessages((items) => [
        ...
        {
          role: 'assistant',
          content: `已完成 ${results.length} 张切片分割。我已经整理好当前分割指标，接下来你问“解读当前分割结果”或“生成科研汇报”时，会把这些真实数据一起发给 DeepSeek。`,
        },
      ])
    } catch (err) {
      setMessages((items) => [
          ...
          <section className="assistant-chat">
            ...
            <div className="starter-row">
              ...
              <button
                ...
                onClick={() => sendMessage('请基于当前真实分割结果，按科研汇报格式总结模型输出、器官面积占比和注意事项。不要编造 Dice、HD95 或临床诊断。')}
              >
                解读当前分割结果
              </button>
            </div>
