...
def selective_scan_ref_fallback(
        u,
        delta,
        A,
        B,
    ...
    """Pure PyTorch selective-scan fallback used when mamba_ssm CUDA ops are unavailable."""
    dtype_in = u.dtype
    u = u.float()
    delta = delta.float()
...
try:
    ...
    selective_scan_fn = selective_scan_ref_fallback
    selective_scan_ref = selective_scan_ref_fallback
...
try:
    ...
    selective_scan_fn_v1 = selective_scan_ref_fallback
    selective_scan_ref_v1 = selective_scan_ref_fallback
...
class MS_SSM(nn.Module):
    ...
    def forward(self, input: torch.Tensor):
        ...
        input_left = self.msdc(input_left)
        x = x.permute(0, 3, 1, 2).contiguous()
