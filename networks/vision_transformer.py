...
from .mmrsg_unet import CSWinTransformer 

logger = logging.getLogger(__name__)

class MMRSGUNet(nn.Module):
    def __init__(self, config, img_size=224, num_classes=21843, zero_head=False, vis=False):
        super(MMRSGUNet, self).__init__()
        self.num_classes = num_classes
        self.zero_head = zero_head
        self.config = config
        self.mmrsg_unet = CSWinTransformer(img_size=config.DATA.IMG_SIZE,
    ...
    def load_from(self, config):
        ...
        if pretrained_path is not None:
            ...
            model_dict = self.mmrsg_unet.state_dict()
            ...
            msg = self.mmrsg_unet.load_state_dict(full_dict, strict=False)
        else:
            print("none pretrain")